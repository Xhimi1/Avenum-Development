...
export function WhatsappHero() {
  return (
    <div aria-hidden className="relative h-72 w-72 md:h-[22rem] md:w-[22rem]">
      ...
      {/* phone */}
      <div className="svc-float absolute inset-x-16 inset-y-1 overflow-hidden rounded-[2.2rem] bg-white p-2.5" style={{ border: `1px solid ${HAIRLINE}`, boxShadow: ELEV_LG }}>
        {/* status bar / header */}
        <div className="mb-2.5 flex items-center gap-2 rounded-t-[1.6rem] px-2 pb-2.5 pt-1" style={{ borderBottom: `1px solid ${HAIRLINE}` }}>
        ...
        {/* incoming */}
        <div className="mb-2 max-w-[72%] rounded-2xl rounded-tl-sm bg-black/[0.05] px-3 py-2" style={{ boxShadow: ELEV_SM }}>
          <div className="h-1.5 w-20 rounded-full bg-black/15" />
          <div className="mt-1.5 h-1.5 w-12 rounded-full bg-black/[0.08]" />
        </div>
        {/* outgoing auto-reply */}
        <div className="mb-2 ml-auto max-w-[74%] rounded-2xl rounded-tr-sm px-3 py-2" style={{ background: `linear-gradient(150deg, ${soft(30)}, ${soft(14)})`, boxShadow: ELEV_SM }}>
        ...
        {/* typing bubble */}
        <div className="ml-auto flex max-w-[40%] items-center gap-1 rounded-2xl rounded-tr-sm px-3 py-2.5" style={{ background: `linear-gradient(150deg, ${soft(30)}, ${soft(14)})`, boxShadow: ELEV_SM }}>
      ...
      <div className="svc-sway absolute right-3 top-8 flex h-14 w-14 items-center justify-center rounded-2xl bg-white" style={{ border: `1px solid ${HAIRLINE}`, boxShadow: ELEV_MD + ', ' + GLOSS, ['--svc-rot' as string]: '5deg' }}>
      ...
      <div className="svc-float absolute -left-1 top-24 flex items-center gap-1.5 rounded-xl bg-white px-2.5 py-1.5" style={{ border: `1px solid ${HAIRLINE}`, boxShadow: ELEV_MD, animationDelay: '-2s' }}>
  ...
  return (
    <div aria-hidden className="flex h-full w-full items-center justify-center">
      <div className="w-44 space-y-2.5">
        <div className="max-w-[80%] rounded-2xl rounded-tl-sm bg-white px-3 py-2" style={{ border: `1px solid ${HAIRLINE}`, boxShadow: ELEV_SM }}>
          <div className="h-1.5 w-24 rounded-full bg-black/12" />
        </div>
        <div className="ml-auto flex max-w-[80%] items-center gap-2 rounded-2xl rounded-tr-sm px-3 py-2" style={{ background: 'var(--svc-accent)', boxShadow: chip('var(--svc-accent)') }}>
