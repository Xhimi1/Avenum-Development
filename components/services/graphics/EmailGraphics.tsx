...
export function EmailHero() {
  return (
    <div aria-hidden className="relative h-72 w-72 md:h-[22rem] md:w-[22rem]">
      ...
      <div className="absolute inset-x-8 top-14 h-40 rounded-2xl bg-white" style={{ border: `1px solid ${HAIRLINE}`, boxShadow: ELEV_SM, transform: 'rotate(-7deg)' }} />
      <div className="absolute inset-x-6 top-10 h-44 rounded-2xl bg-white" style={{ border: `1px solid ${HAIRLINE}`, boxShadow: ELEV_MD, transform: 'rotate(4deg)' }} />

      {/* front email card */}
      <div className="svc-float absolute inset-x-3 top-4 rounded-2xl bg-white p-4" style={{ border: `1px solid ${HAIRLINE}`, boxShadow: ELEV_LG }}>
        <div className="relative mb-3 flex h-24 items-center justify-center overflow-hidden rounded-xl" style={{ background: `linear-gradient(135deg, ${soft(26)}, ${soft(8)})`, boxShadow: GLOSS }}>
          <div className="svc-sheen absolute -inset-y-2 left-0 w-10 bg-white/40 blur-md" />
          <span className="flex h-12 w-12 items-center justify-center rounded-2xl" style={{ background: 'white', boxShadow: ELEV_SM }}>
            <svg viewBox="0 0 24 24" className="h-6 w-6" fill="none" stroke="var(--svc-accent)" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="5" width="18" height="14" rx="2.5" /><path d="M3.5 7.5 12 13l8.5-5.5" /></svg>
          </span>
        </div>
      ...
      {/* floating "sent" pill */}
      <div className="svc-float absolute -left-3 top-12 rounded-xl bg-white px-3 py-2 text-[0.6rem] font-semibold" style={{ border: `1px solid ${HAIRLINE}`, boxShadow: ELEV_MD, color: deep(15), animationDelay: '-2.4s' }}>
        ✓ Dërguar
      </div>
    </div>
  );
      ...
      <div className="flex items-center">
        {[0, 1, 2].map((i) => (
          <div key={i} className="flex items-center">
            <div
              className="svc-pulse flex h-14 w-14 items-center justify-center rounded-2xl bg-white"
              style={{ border: `1px solid ${HAIRLINE}`, boxShadow: ELEV_SM + ', ' + GLOSS, animationDelay: `${i * 0.35}s` }}
            >
              <span className="h-6 w-6 rounded-lg" style={{ background: `linear-gradient(150deg,${soft(34 + i * 8)},${soft(16 + i * 6)})`, boxShadow: '0 2px 4px rgba(0,0,0,0.15)' }} />
            </div>
            {i < 2 && (
...
export function TileInbox() {
  return (
    <div aria-hidden className="flex h-full w-full items-center justify-center">
      <div className="w-48 rounded-2xl bg-white p-3" style={{ border: `1px solid ${HAIRLINE}`, boxShadow: ELEV_MD }}>
        {[70, 55, 42].map((w, i) => (
          <div key={i} className="mb-2 flex items-center gap-2.5 rounded-lg p-1.5 last:mb-0" style={i === 0 ? { background: soft(8) } : undefined}>
            <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full" style={{ background: `linear-gradient(150deg,${soft(24 + i * 8)},${soft(10 + i * 6)})`, boxShadow: ELEV_SM }}>
              <svg viewBox="0 0 24 24" className="h-3.5 w-3.5" fill="none" stroke="white" strokeWidth="1.8"><rect x="3" y="5" width="18" height="14" rx="2.5" /><path d="M4 7l8 5 8-5" /></svg>
            </span>
            <div className="flex-1">
