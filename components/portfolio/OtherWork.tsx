      ...
      <div className="mt-8 border-t border-white/20">
        {others.map((p) => (
          <button
            ...
            className="group flex w-full items-center justify-between gap-6 border-b border-white/20 py-8 text-left transition-colors duration-300 hover:bg-black/[0.02]"
          >
            <h3 className="heading-sm min-w-0 leading-none">
              {p.name}
              {p.isPrototype && (
            ...
            <ArrowRight
              className="h-8 w-8 shrink-0 text-white transition-transform duration-300 group-hover:translate-x-2 md:h-12 md:w-12"
            />
          </button>
        ))}
      </div>
